...
true_file = "test_true.npy"
pred_file = "test_pred.npy"
orig_file = 'test_orig.npy'
tout_file = 'test_out.npy'
appr_file = "apoi_pred.npy"
apla_file = "apoi_label.npy"
rawm_file = "test_raw_max.npy"
...
appr_data = np.load(appr_file,allow_pickle=True)
apla_data = np.load(apla_file,allow_pickle=True)
true_data = np.asarray([i.numpy() for i in true_data])
rawm_data = np.load(rawm_file,allow_pickle=True)
cla5_data = np.load("../test_out_cla5.npy",allow_pickle=True)
cla4_data = np.load("../test_out_cla4.npy",allow_pickle=True)
cla3_data = np.load("../test_out_cla3.npy",allow_pickle=True)
cla1_data = np.load("../test_out_cla1.npy",allow_pickle=True)
cla0_data = np.load("../test_out_cla0.npy",allow_pickle=True)
...
#
print("orig_data:\n",orig_data.shape, orig_data[0])
print("tout_data:\n",tout_data.shape, tout_data[0])
print("cla5_data:\n",cla5_data.shape, cla5_data[0])
#
...
confidences = [i * 0.01 for i in range(1, 100)]
pr_curve = []
print(f"confidences:{confidences}")
cla_datas = [cla0_data, cla1_data, cla3_data, cla4_data, cla5_data]
high_scores = [0.7, 0.8, 0.9]
low_scores = [0.3, 0.2, 0.1]
#high_scores = [0.8]
#low_scores = [0.2]
def rules(i, j):
    rule_scores = []
...
for i in range(m):
    ...
    for j in range(n):
        ...
        tmp_charts += rules(i, j)
        charts.append(tmp_charts)

        #if cla5_data[i,j] > 0.8:
        #    pnew = 5
        #elif cla4_data[i,j] > 0.8:
        #    pnew = 4
        #if cla3_data[i,j] > 0.98:
        #    pnew = 3
        labels_data.append(true_data[i,j])
        single_data.append(pnew)
        if true_data[i,j] != pnew:
            #true_false[true_data[i,j]][pred_data[i,j]].append(orig_data[i,j])
            true_false[true_data[i,j]][pred_data[i,j]].append(rawm_data[i,j])
            #if (tk.values[0] < tk.values[1] + 1):
            #    if ap[tk.indices[0]] < ap[tk.indices[1]]:
            #        pnew = tk.indices[1]
            #        print(tk, true_data[i,j], pnew)
            #print(true_data[i,j], pred_data[i,j], ap , tout)
            count += 1
...
def ruleForPNCorrection(all_charts, epsilon):
    ...
    for count, chart in enumerate(all_charts):
        ...
        #scores_rec = get_scores(rec_true, rec_pred)
        #scores_ori = get_scores(chart[:,1], chart[:,0])
...
epsilon = [0.0001 * i for i in range(1, 1000, 10)]
...
df = pd.DataFrame(results, columns = ['epsilon'] + col * 6)
...
#np.save("single_datas.npy", single_data)

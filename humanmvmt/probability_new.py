...
class Probability:
                ...
                for s in range(n_size):
                    ...
                    for j in range(a.shape[0]):
                        for i in range(a.shape[1]):
                            results[j,i] = self.probability[j,i,:,a[j,i]]
                    features[batch][s].append(results)
    ...
    def set_probability(self):
        ...
        for key, values in dicts.items():
            with open("valid" + str(key) + ".pkl", "wb") as fp:   #Pickling
        ...
        probability = np.zeros((len(all_keys), n_dimension, len(all_keys),len(all_keys)))
        with open("Models/cosine_similarity.pkl", 'rb') as f:
            features = pickle.load(f)
            for k in all_keys:
                counts = [[[ 0 for j in range(len(all_keys)) ] for i in range(n_dimension)] for _ in range(len(all_keys))]
                feature = features[k]
                for f in feature:
                ...
                for jj in range(len(all_keys)):
                    for ii in range(n_dimension):
                        for kk, vv in enumerate(counts[jj][ii]):
                            probability[jj][ii][w1][kk] = vv/length
                        #probability[ii][w1][kk] = vv
        ...
        for j in range(len(all_keys)):
            for i in range(n_dimension):
                probability[j,i] = probability[j,i] / np.sum(probability[j,i], axis = 0)
